...
fun_odds.ratio.by.EUR = function(data, DEPENDENT, 
  ...
  colnames(mat_confounding) = c('EUR_sub', 'tags_ancestry')
  
  ## Arranging data for plot:
  dd = data %>%
    mutate(EUR_sub = Condition) %>%
  ...
  if (max(dd$values.high, na.rm = T)>max_y){
    dd[which(dd$values.high > max_y), 'values.high'] = max_y
  }
  if (min(dd$values.low, na.rm = T)<min_y | is.na(min(dd$values.low))){
    dd[which(dd$values.low < min_y), 'values.low'] = min_y
  }
  if (max(dd$values.plot, na.rm = T)>max_y){
    dd[which(dd$values.plot > max_y), 'values.plot'] = max_y
  }
  if (min(dd$values.plot, na.rm = T)<min_y){
