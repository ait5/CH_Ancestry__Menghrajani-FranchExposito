...
fun_ht.ch.Gene.v.Ancestry <- function(x_file, v_conditions, ch.genes,
  ...
  ## Heatmap 
  ht = ggplot(m3 ,
              aes(x=reorder(text.Condition_tags, -n_Condition), y=reorder(Gene, -order_ch), fill= values.plot)) + 
  ...
  if (add_numbers == TRUE) {
    ht = ht +
      geom_text(aes(label=n_Dependent_in_Condition), size=1.5, position = position_nudge(y = -0.2), color='gray20') 
  }
  ht = ht + 
    geom_tile(data=m3,
