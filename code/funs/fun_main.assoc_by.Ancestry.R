...
fun_main.assoc_by.Ancestry = function(df_ano, DEPENDENT = 'functional_ch',
                                      v_confounders_glm=NULL){
  ...
  if (DEPENDENT == 'functional_ch') {
    
    data = df_ano %>%
      select(all_of(DEPENDENT), ancestry_label, all_of(v_confounders_glm)) %>%
      left_join(mat_confounding, by='ancestry_label')
    
  } else if (DEPENDENT == 'more_than_one_ch_muts_functional') {
    
    data = df_ano  %>% 
      filter(more_than_one_ch_muts_functional == 1 | functional_ch == 0 | ch_status == 0 ) %>%
      select(all_of(DEPENDENT), ancestry_label, all_of(v_confounders_glm)) %>%
      left_join(mat_confounding, by='ancestry_label')
    
  } else if (DEPENDENT == 'functional_ch_pd') {
    
    data = df_ano  %>% 
      filter(functional_ch_pd == 1 | functional_ch == 0 | ch_status == 0 ) %>%
      select(all_of(DEPENDENT), ancestry_label, all_of(v_confounders_glm)) %>%
      left_join(mat_confounding, by='ancestry_label')
    
  } else if (DEPENDENT == 'HighVAF_CH') {
    
    data = df_ano  %>% 
      filter(HighVAF_CH == 1 | functional_ch == 0 | ch_status == 0 ) %>%
      select(all_of(DEPENDENT), ancestry_label, all_of(v_confounders_glm)) %>%
      left_join(mat_confounding, by='ancestry_label')
  ...
  # Glm.test = glm(get(DEPENDENT) ~ order_ancestry + age + Sex + smoking_bin + binary_therapy + Major.Cancer.Type,
  #                data=data2)
