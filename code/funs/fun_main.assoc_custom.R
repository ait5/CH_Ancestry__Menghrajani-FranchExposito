...
fun_main.assoc_custom = function(df_ano, DEPENDENT,
                                      mat_confounding,
                                      v_confounders_glm){
  
  
  ####################################################################
  ####### DEPENDENT ~ Ancestry : Reference on first tag ####################
  ...
  v_ancestry_label = mat_confounding$sub_ancestry
  names(v_ancestry_label) = mat_confounding$tags_sub_ancestry
  ...
  b_multi_3 = df_numbers_by.ancesty %>%
    ...
    filter(Condition_tags %in% mat_confounding$tags_sub_ancestry) %>%
    select(Condition, Dependent, Total, n_Condition, n_Dependent, n_Dependent_in_Condition,
           f_Dependent_in_Total, f_Dependent_in_Condition, Condition_tags) %>%
