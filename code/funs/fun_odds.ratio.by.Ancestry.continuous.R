...
fun_odds.ratio.by.Ancestry.continuous = function(data,
                                      size_text=2, color_p='gray',
                                      odds_limits=c(-0.3, 0.3), by_scale=0.1){
  
  ## Arranging data for plot:
  dd = data %>%
    mutate(tags_ancestry = Condition_tags) %>%
    ...
    mutate(text_condition = tags_ancestry) %>%
  ...
  p_odds <- ggplot(dd, aes(y = reorder(tags_ancestry, -order), x = values.plot) ) +
    ...
    #                       label= as.character(text_condition)),
    #           size=size_text,
